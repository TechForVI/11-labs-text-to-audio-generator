    ...
    pcall(function()
        Http.get(VERSION_URL, function(code, response)
            if code == 200 and response then
                local onlineVersionStr = trim(response)
                local onlineVersion = tonumber(onlineVersionStr)
                local currentVer = tonumber(CURRENT_VERSION)
                
                -- یہ لاجک دونوں ورژنز کا نمبرز میں موازنہ کرے گی، جس سے بار بار اپڈیٹ کا مسئلہ حل ہو جائے گا
                if onlineVersion and currentVer and onlineVersion > currentVer then
                    Http.get(UPDATE_CODE_URL, function(code2, mainCode)
                        if code2 == 200 and mainCode and trim(mainCode) ~= "" then
                            ...
                            updateAlertDlg.setMessage("A new version (" .. onlineVersionStr .. ") is available. Would you like to update now?")
                            updateAlertDlg.setButton("Update Now", function()
                                updateAlertDlg.dismiss()
                                performUpdate(mainCode, onlineVersionStr)
                            end)
